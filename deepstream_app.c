...
/*-------------------------------------*/
/* 문자열 붙이기 함수 안전하게 구현 */
#define REMAIN() (128 - (str_ins_pos - obj->text_params.display_text))

      /* obj label */
      if (obj->obj_label[0] != '\0')
      {
        snprintf(str_ins_pos, REMAIN(), "%s", obj->obj_label);
      ...
      if (obj->object_id != UNTRACKED_OBJECT_ID)
      {
        if (appCtx->config.tracker_config.display_tracking_id)
        {
          ...
          snprintf(str_ins_pos, REMAIN(), " %lu",
                   (obj->object_id & LOW_32_MASK));
          str_ins_pos += strlen(str_ins_pos);
        }
      }
           ...
           l_class = l_class->next)
      {
             ...
             l_label = l_label->next)
        {
          ...
          {
            snprintf(str_ins_pos, REMAIN(), " %s", label->pResult_label);
          }
          else if (label->result_label[0] != '\0')
          {
            snprintf(str_ins_pos, REMAIN(), " %s", label->result_label);
          }
                       ...
                       bbox_generated_callback bbox_generated_post_analytics_cb)
{
  ...
  *sink_elem = *src_elem = NULL;
  ...
  /* ADD TRACKER OUTPUT PROBE (correct location) */
                ...
                overlay_graphics_callback overlay_graphics_cb)
{
  ...
  // Implementing a counter
  /* ADDED: initialize count manager and mqtt client */
  count_manager_init();
...
// Implementing a counter
/* ADDED: tracker probe callback */
// GstPadProbeReturn tracker_buf_prob(GstPad *pad, GstPadProbeInfo *info, gpointer user_data)
// {
//   printf("[DEBUG] tracker_buf_prob: ENTER\n");

//   GstBuffer *buf = GST_PAD_PROBE_INFO_BUFFER(info);
//   if (!buf)
//   {
//     printf("[DEBUG] buf NULL\n");
//     return GST_PAD_PROBE_OK;
//   }
...
//   if (!batch_meta)
//   {
//     printf("[DEBUG] batch_meta NULL\n");
//     return GST_PAD_PROBE_OK;
//   }

//   printf("[DEBUG] Start iter frames\n");

//   for (NvDsMetaList *l_frame = batch_meta->frame_meta_list; l_frame; l_frame = l_frame->next)
//   {
//     NvDsFrameMeta *frame_meta = (NvDsFrameMeta *)l_frame->data;
//     printf("[DEBUG] Frame meta OK\n");

//     for (NvDsMetaList *l_obj = frame_meta->obj_meta_list; l_obj; l_obj = l_obj->next)
//     {
//       NvDsObjectMeta *obj_meta = (NvDsObjectMeta *)l_obj->data;
//       printf("[DEBUG] obj: class=%d id=%lu\n",
//              obj_meta->class_id, obj_meta->object_id);

//       count_manager_process_obj(obj_meta->class_id, obj_meta->object_id);

//       printf("[DEBUG] count_manager_process_obj OK\n");
//     }
//   }

//   unsigned long long total = count_manager_get_total();
//   printf("[DEBUG] total=%llu\n", total);

//   mqtt_client_publish_uint64("deepstream/count/total", total);
//   printf("[DEBUG] publish OK\n");

//   printf("[DEBUG] tracker_buf_prob: EXIT\n");
//   return GST_PAD_PROBE_OK;
// }
